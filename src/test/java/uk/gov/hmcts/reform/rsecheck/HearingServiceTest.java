...
class HearingServiceTest {
    ...
    private Hearing createHearing(UUID id) {
        ...
        return new Hearing(
            ...
            UUID.randomUUID(),      // heaaringTypeId
    ...
    void shouldReturnAllHearingsMappedToDtos() {
        ...
        when(mockRepository.findAll()).thenReturn(List.of(hearingEntity)); //red squiggle - Cannot access org
        // .springframework.data.jpa.repository.JpaRepository

        // Act
        results = service.getAllHearings();

        assertThat(results).isNotNull();
        assertThat(results).hasSize(1);
        
        assertThat(results.get(0).getId()).isEqualTo(id);
        assertThat(results.get(0).getEstimatedMins()).isEqualTo(35);
        assertThat(results.get(0).getJurisdictionType()).isEqualTo("TestJurisdiction");
        assertThat(results.get(0).getHearingLanguage()).isEqualTo("English");
        assertThat(results.get(0).getBoxHearing()).isEqualTo(true);
